...
//                        $saved_shipping_zones = get_option('tpbs_shipping_zone', array());
//                        var_dump($saved_payment_settings);
//                        exit;
                        ...
                        if (!empty($saved_payment_settings)) {
                            foreach ($saved_payment_settings as $key => $payment_setting) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label class="toggle-switch">
                                            <input type="hidden" name="payment_visibility_h[]" id="<?php echo 'payment_visibility_h_' . $key; ?>"
                                                   value="<?php echo ($payment_setting['visibility'] === 'hide') ? 'hide' : 'show'; ?>">
                                            <input type="checkbox" name="payment_visibility[]"
                                                   value="<?php echo ($payment_setting['visibility'] === 'hide') ? 'hide' : 'show'; ?>"
                                                <?php checked('hide', $payment_setting['visibility']); ?>
                                                   id="<?php echo 'payment_visibility_' . $key; ?>"
                                                   onchange="toogleSwitch(this.value, <?php echo $key; ?>)">
                                            <span class="slider"></span>
                                        </label>
                                        <span class="toggle-comment">
                    <?php echo $payment_setting['visibility'] === 'hide' ? 'Hide Payment' : 'Show Payment'; ?>
                </span>
                                    </td>
                                    <td>
                                        <button type="button" class="delete-button">Delete</button>
                    ...
                    <div class="save-container">
                        <?php wp_nonce_field('tpbs_update_settings'); ?>
                        <input type="hidden" name="action" value="tpbs_update_settings">
                        <?php submit_button(__('Save Settings', 'toggle-payment-by-shipping'));
                        ?>
                    </div>
                </div>
            </form>
    ...
    /**
     * Updates the payment settings based on form submission.
     *
     * This method processes the submitted form data, sanitizes it,
     * and updates the settings in the database. It performs a security check
     * using a nonce to ensure the request is valid.
     *
     * @since 1.0.0
     */

    /**
     * Updates the payment settings and shipping zones based on form submission.
     *
     * This method processes the submitted form data, sanitizes it,
     * and updates the settings in the database. It performs a security check
     * using a nonce to ensure the request is valid.
    ...
    public function update_settings() {
        if (!check_admin_referer('tpbs_update_settings')) {
            wp_die(esc_html__('Security check failed', 'toggle-payment-by-shipping'));
        }

        // Check if arrays are set
        $shipping = isset($_POST['tpbs_shipping_region']) ? array_map('sanitize_text_field', $_POST['tpbs_shipping_region']) : array();
        $methods = isset($_POST['tpbs_payment_method']) ? array_map('sanitize_text_field', $_POST['tpbs_payment_method']) : array();
        $visibilities = isset($_POST['payment_visibility_h']) ? array_map('sanitize_text_field', $_POST['payment_visibility_h']) : array();

        // Initialize arrays for payment settings and shipping zones
        $payment_settings = array();
        $shipping_zones = array();

        for ($i = 0; $i < count($shipping); $i++) {
            // Store payment method and visibility in tpbs_payment_settings
            $payment_settings[] = array(
                'shipping_region' => $shipping[$i],
                'method' => isset($methods[$i]) ? $methods[$i] : '',
                'visibility' => isset($visibilities[$i]) ? $visibilities[$i] : 'show',
            );
        }

        // Save payment settings to tpbs_payment_settings option
        update_option('tpbs_payment_settings', $payment_settings);

        // Save shipping zones to tpbs_shipping_zone option
