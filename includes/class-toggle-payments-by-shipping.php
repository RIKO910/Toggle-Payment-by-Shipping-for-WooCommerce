...
class Toggle_Payments_By_Shipping{
    ...
    public function init()
    {
        ...
        add_filter('woocommerce_available_payment_gateways', [$this, 'shipping_checkout_payment']);
...
//        add_action('wp_enqueue_scripts', array($this, 'enqueue_custom_js'));
    ...
    public function enqueue_custom_scripts() {
        ...
        wp_localize_script('tpbc_custom_scripts', 'MYajax', array(
    ...
    public function shipping_checkout_payment($available_gateways) {
        ...
        $payment_settings = get_option('tpbs_payment_settings', []);
    ...
    // Function to handle the AJAX GET request
    function get_shipping_state_data() {
        // Check nonce for security

        ob_start();
        if ( ! isset($_GET['nonce']) || ! wp_verify_nonce($_GET['nonce'], 'custom_checkout_nonce') ) {
            wp_send_json_error(array('message' => 'Nonce verification failed'));
            wp_die();
        }

        // Sanitize and retrieve the selected state
        $selected_state = sanitize_text_field($_GET['state']);

        // Retrieve all checkout settings
        $allCheckoutSettings = get_option('tpbs_payment_settings', []);

        // Filter the settings based on the selected state
        $filtered = array_filter($allCheckoutSettings, function($setting) use ($selected_state) {
        ...
        $matched_setting = !empty($filtered) ? reset($filtered) : [];

        // Prepare the response data
        $response_data = array(
            "success" => true,  // Make sure to set this to true for a successful response
            "data" => array(
